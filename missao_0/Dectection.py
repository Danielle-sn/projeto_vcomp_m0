...
# RETR_TREE → retorna contornos + hierarquia
    ...
    hierarquia = hierarquia[0] if hierarquia is not None else []
    ...
    for i, cnt in enumerate(contornos):
        ...
        if len(approx) == 4 and area > 500: # área em pixels, calcular qual valor seria ideal 
            ...
            if 0.95 <= proporcao <= 1.05:
                ...
                _, _, _, parent = hierarquia[i]

                # ex: Se não tem pai -> quadrado externo
                if parent == -1 or area == max(cv2.contourArea(c) for c in contornos):
                
                    cor = (0,255,0)
                texto = f"Externo (area={int(area)})"
            else:
                    cor = (255,0,0)
                    texto = f"Interno (area={int(area)})"

            cv2.drawContours(frame, [approx], -1, cor, 2)
